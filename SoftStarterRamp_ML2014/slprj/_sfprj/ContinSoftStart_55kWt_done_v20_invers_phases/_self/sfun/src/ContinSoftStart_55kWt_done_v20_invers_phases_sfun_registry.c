  ...
  const mxArray * prhs[] )
{
  ...
  if (sf_ContinSoftStart_55kWt_done_v20_invers_phases_process_check_sum_call
      (nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_ContinSoftStart_55kWt_done_v20_invers_phases_autoinheritance_info( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "ContinSoftStart_55kWt_done_v20_invers_phases") == 0)
  {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_ContinSoftStart_55kWt_done_v20_invers_phases_autoinheritance_info
      (nlhs,plhs,3,newRhs);
  }
  ...
  plhs[], int nrhs, const mxArray * prhs[] )
{
  ...
  if (strcmp(machineName, "ContinSoftStart_55kWt_done_v20_invers_phases") == 0)
  {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_ContinSoftStart_55kWt_done_v20_invers_phases_third_party_uses_info
      (nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int
    sf_ContinSoftStart_55kWt_done_v20_invers_phases_updateBuildInfo_args_info
...
extern unsigned int
  sf_ContinSoftStart_55kWt_done_v20_invers_phases_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"ContinSoftStart_55kWt_done_v20_invers_phases")) {
    return(sf_ContinSoftStart_55kWt_done_v20_invers_phases_method_dispatcher
...
extern void ContinSoftStart_55kWt_done_v20_invers_phases_terminator(void);
void sf_machine_global_terminator(void)
{
  sfGlobalMdlStartCallCounts--;
  if (sfGlobalMdlStartCallCounts == 0) {
    ContinSoftStart_55kWt_done_v20_invers_phases_terminator();
...
extern void ContinSoftStart_55kWt_done_v20_invers_phases_initializer(void);
extern void
  ContinSoftStart_55kWt_done_v20_invers_phases_register_exported_symbols
  (SimStruct* S);
extern void ContinSoftStart_55kWt_done_v20_invers_phases_debug_initialize(struct
  SfDebugInstanceStruct*);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  ContinSoftStart_55kWt_done_v20_invers_phases_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      ContinSoftStart_55kWt_done_v20_invers_phases_debug_initialize
        (sfGlobalDebugInstanceStruct);
    }

    ContinSoftStart_55kWt_done_v20_invers_phases_initializer();
  }
